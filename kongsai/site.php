...
class Site extends IController
{
    ...
    private $redirecturl='http://www.iwebshop.com/index.php';
	...
	function login(){
	    //调取接口获取QQcode码
        $state=md5(rand(1,99999));
	    $url="https://graph.qq.com/oauth2.0/authorize?response_type=code&client_id=$this->appid&redirect_uri=$this->redirecturl&state=$state";
	     header("location:$url");
    }
	function index()
	{
	    //QQ第三方登录
        //接收code码之后会跳转回调index.php 接受code的值
	    if (isset($_GET['code'])){
	        $code=$_GET['code'];
	        //接受到code值之后再调用接口获取token值
	        $url="https://graph.qq.com/oauth2.0/token?grant_type=authorization_code&client_id=$this->appid&client_secret=$this->appkey&code=$code&redirect_uri=$this->redirecturl";
            $tok=file_get_contents($url);
            $left=strpos($tok,'=');
            $right=strpos($tok,'&');
            $token=substr($tok,$left+1,$right-$left-1);
            //token值获取后再调用接口获取openid
            $url="https://graph.qq.com/oauth2.0/me?access_token=$token";
            $openid=file_get_contents($url);
            $left=strpos($openid,'(');
            $right=strpos($openid,')');
            $openid=substr($openid,$left+1,$right-$left-1);
            $data=json_decode($openid,true);
            $openid=$data['openid'];
            //获取openid后再调取接口获取用户infor
            $url="https://graph.qq.com/user/get_user_info?access_token=$token&oauth_consumer_key=$this->appid&openid=$openid";
            $userinfor=file_get_contents($url);//userinfor即是第三方QQ的用户信息

            $gender=json_decode($userinfor,true);
...
//        var_dump($city);die;
    ...
    public  function reg_ok(){
        ...
        $time=time();
        $title='用户激活';
        $url="<a href='http://www.iwebshop.com/index.php?controller=site&action=activation&id=$id_new&t=$time'>http://www.iwebshop.com/index.php?controller=site&action=activation&id=$id_new&t=$time</a>";
        $connect="尊敬的用户您好,欢迎您注册iweb尚无平台,请单击以下链接进行激活,激活后方可正常登陆".$url;
            if($mail->send($email,$title,$connect)){
            echo "已成功发送邮件,请前往激活";
        }else{
            echo $mail->getError();
        }
